...
const PRESET_ROUTES = [
  { id: 'r1', label: 'Lekki Phase 1 → Victoria Island', traffic: 'Moderate', speed: '24km/h' },
  { id: 'r2', label: 'Ajah → Sangotedo', traffic: 'Heavy', speed: '12km/h' },
  { id: 'r3', label: 'Oshodi → Ikeja', traffic: 'Light', speed: '45km/h' },
  { id: 'r4', label: 'Yaba → Surulere', traffic: 'Moderate', speed: '20km/h' },
  { id: 'r5', label: 'Ikeja GRA → Maryland', traffic: 'Light', speed: '38km/h' },
];
...
export default function DriverHome() {
  ...
  // KYC Gate (Redirect if not approved)
  useEffect(() => {
    if (kycStatus === 'NOT_STARTED') {
      ...
      navigate('/kyc/expired', { replace: true });
    }
  }, [kycStatus, navigate]);

  // Working Balance Glow Effect
  useEffect(() => {
    if (wallets.working.lastCreditedAt) {
      setWalletGlow(true);
      const timer = setTimeout(() => setWalletGlow(false), 2000);
      return () => clearTimeout(timer);
    }
  }, [wallets.working.lastCreditedAt]);
  ...
  const confirmOnline = () => {
    ...
    toast.success('System Online — Happy Earnings');
  };

  const confirmOffline = () => {
    setIsOnline(false);
    setShowOfflineConfirm(false);
    toast('System Offline', { icon: '😴' });
  };
      ...
      <div className="absolute top-0 left-0 right-0 z-20 pt-safe bg-gradient-to-b from-[var(--color-bg)] via-[var(--color-bg)]/60 to-transparent pb-16 pointer-events-none">
        ...
        <div className="px-6 pointer-events-auto">
          <div className={`flex bg-[var(--color-surface-0)]/80 backdrop-blur-2xl rounded-2xl p-1 shadow-xl border transition-all duration-700 ${walletGlow ? 'border-[var(--color-primary)] shadow-[0_0_25px_rgba(127,255,0,0.2)] scale-[1.01]' : 'border-[var(--color-surface-3)]/50'}`}>
            <div className="flex-1 px-3 py-2 border-r border-[var(--color-surface-3)]/30 cursor-pointer active:bg-[var(--color-surface-2)] transition-colors rounded-xl" onClick={() => navigate('/wallet/working')}>
              <p className="text-[9px] font-bold text-[var(--color-text-muted)] uppercase tracking-widest mb-0.5">Working</p>
              <p className="text-sm font-display font-bold text-[var(--color-earnings)]">{formatCurrency(wallets.working.balance)}</p>
            </div>
            <div className="flex-1 px-3 py-2 border-r border-[var(--color-surface-3)]/30 cursor-pointer active:bg-[var(--color-surface-2)] transition-colors rounded-xl" onClick={() => navigate('/wallet/main')}>
              <p className="text-[9px] font-bold text-[var(--color-text-muted)] uppercase tracking-widest mb-0.5">Savings</p>
              <p className="text-sm font-display font-bold text-white">{formatCurrency(wallets.main.balance)}</p>
            </div>
            <div className="flex-1 px-3 py-2 cursor-pointer active:bg-[var(--color-surface-2)] transition-colors rounded-xl" onClick={() => navigate('/wallet/maintenance')}>
              <p className="text-[9px] font-bold text-[var(--color-text-muted)] uppercase tracking-widest mb-0.5">Maint.</p>
              <p className="text-sm font-display font-bold text-white">{formatCurrency(wallets.maintenance.balance)}</p>
            </div>
          </div>
        </div>
