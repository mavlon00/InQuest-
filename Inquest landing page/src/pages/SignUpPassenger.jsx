...
import { ShieldCheck, ArrowRight, Eye, EyeOff, AlertCircle, Loader2, User } from 'lucide-react';
...
export default function SignUpPassenger() {
  ...
  const redirectUser = (data) => {
    ...
    const path = (!user.first_name || !user.last_name) ? '/profile-setup' : '/home';
    window.location.href = `${PASSENGER_URL}${path}?token=${encodeURIComponent(access_token)}`;
  };
  ...
  const handleGoogle = () => {
    ...
    // Store intended role + referral in sessionStorage so callback can use it
    sessionStorage.setItem('signup_role', 'Passenger');
    sessionStorage.setItem('signup_referral', form.referral_code || '');
  ...
  const Field = ({ label, id, type = 'text', value, onChange, placeholder, required = true, children }) => (
    <div>
      ...
      <div className="relative">
        <input
          ...
          className="w-full bg-surface-1 border border-surface-2 rounded-xl px-4 py-3 text-text-primary focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors pr-12"
        />
        {children}
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6 py-24 bg-surface-1">
      <div className="w-full max-w-md bg-surface-0 rounded-3xl border border-surface-2 shadow-2xl p-8 md:p-12">
        ...
        <h2 className="text-3xl font-display font-bold text-text-primary text-center mb-1">Create your account</h2>
        <p className="text-text-secondary text-center mb-8">Sign up as a passenger</p>
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <input type="text" value={form.referral_code} onChange={set('referral_code')} placeholder="e.g. ABC123"
              className="w-full bg-surface-1 border border-surface-2 rounded-xl px-4 py-3 text-text-primary focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors uppercase tracking-widest" />
          </div>
        ...
        <div className="mt-6 pt-6 border-t border-surface-2 text-center">
          ...
          <p className="text-text-secondary text-sm mt-2">
            ...
            <Link to="/signup/driver" className="text-primary font-bold hover:underline">Apply to drive</Link>
          </p>
        </div>
      </div>
    </div>
