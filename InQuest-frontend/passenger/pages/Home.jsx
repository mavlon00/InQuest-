...
export default function Home() {
  ...
  const { fetchWalletData, fetchProfileData } = useStore();
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          updateBooking({ passengerLat: latitude, passengerLng: longitude });
        },
        (error) => {
          console.error("Error getting location", error);
          // Fallback to a default if necessary
          const defaultLoc = [6.5244, 3.3792];
          setLocation(defaultLoc);
          updateBooking({ passengerLat: defaultLoc[0], passengerLng: defaultLoc[1] });
        }
      );
    }
  }, [updateBooking]);

  // Vehicle movement simulation removed to focus on real data.
        ...
        <div className="bg-[var(--color-surface-1)] rounded-[var(--radius-xl)] border border-[var(--color-border-subtle)] p-4 flex items-center justify-between shadow-md">
          ...
          >
            ...
            <div>
              ...
              <p className="font-display font-semibold text-lg text-[var(--color-success)] leading-tight">{user?.green_points || 0}</p>
            </div>
          </div>
