...
export default function PersonalBookingTracking() {
  ...
  const {
    ...
    waitingFeeActive, waitingFeeAmount, fareEstimate, driverLocation,
    updateBookingStatus, updateWaitingFee, startWaitingFee, resetBooking,
  } = useBookingStore();

  const [displayLocation, setDisplayLocation] = useState(driverLocation || { lat: 0, lng: 0, heading: 0 });
  ...
  // Sync display location with store's driverLocation
  useEffect(() => {
    if (driverLocation) {
      setDisplayLocation(driverLocation);
    }
  }, [driverLocation]);

  // Real status polling fallback
  useEffect(() => {
      ...
      if (bookingId) {
        try {
          ...
          if (res.data.status === 'success') {
            ...
            if (ride.status !== bookingStatus) {
              updateBookingStatus(ride.status);
            }
          }
  ...
  useEffect(() => {
    ...
    if (bookingStatus === 'IN_PROGRESS') {
      interval = setInterval(() => setTripDuration(d => d + 1), 1000);
    }
  ...
  useEffect(() => {
    // This effect is now primarily for smooth driver icon movement based on driverLocation from store
    // The actual targetLocation logic is handled by the backend/websocket updates to driverLocation
    if (!driverLocation || !displayLocation) return;
    
    let startTime = null;
    const duration = 2000; // 2 seconds for interpolation
    const startLoc = { ...displayLocation };
    const targetLoc = { ...driverLocation }; // Use driverLocation from store as target
    ...
    const animate = (time) => {
      ...
      setDisplayLocation({
        lat: startLoc.lat + (targetLoc.lat - startLoc.lat) * progress,
        lng: startLoc.lng + (targetLoc.lng - startLoc.lng) * progress,
        heading: targetLoc.heading // Heading should be directly from driverLocation
      });
    ...
    requestAnimationFrame(animate);
  }, [driverLocation]); // Depend on driverLocation from store
