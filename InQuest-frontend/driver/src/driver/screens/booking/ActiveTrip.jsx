...
export default function ActiveTrip() {
  ...
  // Status transitions are now handled exclusively by API calls and WebSocket updates.
  // No fake timeouts needed for production.
  ...
  const totalFare = isCash && iotOn
    ...
    : (activeTrip.estimatedFare || 0) + waitingFee;
  ...
  const handleArrived = async () => {
    ...
    try {
      const response = await api.put(`/rides/${activeTrip.id}/arrived`);
  ...
  const handleStartTrip = async () => {
    ...
    try {
      const response = await api.put(`/rides/${activeTrip.id}/start`);
      if (response.data.status === 'success') {
  ...
  const handleEndTrip = async () => {
    setEndLoading(true);
    try {
      const response = await api.put(`/rides/${activeTrip.id}/end`);
      if (response.data.status === 'success') {
  ...
  const handleCashCollected = async () => {
    try {
      const response = await api.post(`/rides/${activeTrip.id}/cash-collected`);
      if (response.data.status === 'success') {
        setCashCollected(true);
        toast.success('Cash collection confirmed');
      }
    } catch (err) {
      toast.error(err?.response?.data?.message || 'Could not confirm cash collection');
    }
  };
  ...
  const handleDone = async () => {
    ...
    setDoneLoading(true);
    try {
      await api.post(`/rides/${activeTrip.id}/rate`, { rating, comment: '' });
  ...
  const handleConfirmCancel = useCallback(async () => {
    ...
    setCancelError('');
    try {
      await api.put(`/rides/${activeTrip.id}/cancel`, { reason: selectedReason });
      ...
      setCancelError(err?.response?.data?.message || 'Cancel failed. Please try again.');
    } finally {
      setCancelLoading(false);
    }
  }, [selectedReason, activeTrip?.id, setActiveTrip, setTripStatus, navigate]);
        ...
        <div className="bg-[var(--color-surface-0)]/95 backdrop-blur-md rounded-[var(--radius-md)] p-3 border border-[var(--color-surface-3)] flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-[var(--color-surface-3)] flex items-center justify-center font-display font-bold shrink-0">
              {activeTrip.passenger?.name?.charAt(0) || 'P'}
            </div>
            <div>
              <p className="font-semibold text-sm">{activeTrip.passenger?.name || 'Passenger'}</p>
              <div className="flex items-center gap-2 text-xs text-[var(--color-text-secondary)]">
                <span className="bg-[var(--color-surface-2)] px-2 py-0.5 rounded-sm">{activeTrip.paymentMethod}</span>
                <span className="font-display font-bold text-[var(--color-earnings)]">₦{(activeTrip.estimatedFare || 0).toLocaleString()}</span>
              </div>
            </div>
          </div>
              ...
              <div className="flex bg-[var(--color-surface-2)] rounded-[var(--radius-md)] p-4 mb-4 gap-4">
                ...
                <div className="text-center flex-1">
                  ...
                  <p className="font-display font-bold text-lg text-[var(--color-earnings)]">
                    ₦{((activeTrip.estimatedFare || 0) + waitingFee).toLocaleString()}
                  </p>
                </div>
              </div>
              ...
              {isCash && (
                <div className="mb-4 text-center bg-[var(--color-surface-2)] rounded-[var(--radius-md)] p-4 border border-[var(--color-primary)]/20">
                  ...
                  <p className="font-display font-bold text-3xl text-[var(--color-earnings)]">
                    ₦{(iotOn ? activeCashTrip.iotCurrentFare : (activeTrip.estimatedFare || 0) + waitingFee).toLocaleString()}
                  </p>
            ...
            <div className="bg-[var(--color-surface-1)] rounded-[var(--radius-lg)] p-5 mb-6 border border-[var(--color-surface-3)]">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <span>₦{(activeTrip.estimatedFare || 0).toLocaleString()}</span>
                </div>
                {waitingFee > 0 && (
                  <div className="flex justify-between">
