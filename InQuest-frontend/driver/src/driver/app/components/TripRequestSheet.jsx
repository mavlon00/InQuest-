...
export default function TripRequestSheet() {
  ...
  }, [incomingRequest, setRequestCountdown]);
  ...
  const handleAccept = async () => {
    ...
    try {
      const response = await api.put(`/rides/${incomingRequest.tripId}/accept`);
      if (response.data.status === 'success') {
        const rideData = response.data.data;
        setActiveTrip({ ...incomingRequest, ...rideData, status: 'CONFIRMED' });
        setTripStatus('EN_ROUTE');
        setIncomingRequest(null);
        
        // Register match in backend ConnectionManager for real-time location broadcasting
        try {
          const { sendSocketMessage } = await import('../../utils/websocket');
          sendSocketMessage({
            type: 'register_match',
            passengerId: incomingRequest.passenger.id || incomingRequest.passengerId
          });
        } catch (e) { console.error("WS Map error", e); }
      ...
      toast.error(err?.response?.data?.message || 'Failed to accept trip. Try again.');
    } finally {
  ...
  const handleConfirmDecline = useCallback(async () => {
    ...
    setDeclineError('');
    try {
      await api.put(`/rides/${incomingRequest.tripId}/decline`, { reason: selectedReason });
      setIncomingRequest(null);
      setView('main');
      toast('Trip declined.', { icon: '✕' });
    } catch (err) {
      console.error('Decline error:', err);
      setDeclineError(err?.response?.data?.message || 'Failed to decline. Try again.');
    } finally {
      setDeclineLoading(false);
    }
  }, [selectedReason, incomingRequest, setIncomingRequest]);

  if (!incomingRequest) return null;

  const pct      = (requestCountdown / (incomingRequest.timeoutSecs || 25)) * 100;
